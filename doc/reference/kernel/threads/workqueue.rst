...
.. note::
   ...
   implementation used prior to release 2.5.  Among the changes are:
   ...
   * Safe handling of work item resubmission when the item is being processed
   ...
   Using the return values of :c:func:`k_work_busy()` or
   :c:func:`k_work_pending()`, or measurements of remaining time until
   delayable work is scheduled should be avoided to prevent race conditions of
   the type observed with the previous implementation.
...
A work item has is assigned a **handler function**, which is the function
...
* :c:func:`k_work_busy()` returns a snapshot of flags indicating work item
  ...
  infrastructure.
* :c:func:`k_work_pending()` is a helper that indicates ``true`` if and only
  if the work is not scheduled, pending, or being executed.
...
* :c:func:`k_work_cancel()` attempts to prevent the work item from being
  executed.  This may or may not be successful.
...
* :c:func:`k_work_delayable_busy()` is the analog to :c:func:`k_work_busy()`
  for delayable work.
* :c:func:`k_work_delayable_pending()` is the analog to
  :c:func:`k_work_pending()` for delayable work.
