...
* GPIO
  ...
  * The gpio_pin_write() and gpio_pin_read() functions have been, and
    should be replaced with gpio_pin_set_raw() and gpio_pin_get_raw(),
    or if the active level flags were present and supported by
    gpio_pin_set() and gpio_pin_get().
  * The gpio_pin_enable_callback() function has been replaced by
    gpio_pin_interrupt_configure(), passing an enabling interrupt
    configuration.
  * The gpio_pin_enable_callback() function has been replaced by
    gpio_pin_interrupt_configure(), passing GPIO_INT_DISABLE.
