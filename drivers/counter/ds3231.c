...
int rtc_ds3231_ctrl_update(struct device *dev,
			   u8_t set_bits,
			   u8_t clear_bits)
{
	struct ds3231_data *data = dev->driver_data;
...
static inline int rsc_stat(struct device *dev,
			   ...
			   u8_t clear)
{
	u8_t const ign = RTC_DS3231_REG_STAT_OSF | RTC_DS3231_ALARM1
			 | RTC_DS3231_ALARM2;
	struct ds3231_data *data = dev->driver_data;
	const struct ds3231_config *cfg = dev->config->config_info;
	struct register_map *rp = &data->registers;
...
int rtc_ds3231_stat_update(struct device *dev,
			   u8_t set_bits,
			   u8_t clear_bits)
{
	struct ds3231_data *data = dev->driver_data;
...
int rtc_ds3231_get_alarm(struct device *dev,
			 u8_t id,
...
int rtc_ds3231_set_alarm(struct device *dev,
			 u8_t id,
			 const struct rtc_ds3231_alarm *cp)
{
	struct ds3231_data *data = dev->driver_data;
...
int rtc_ds3231_check_alarms(struct device *dev)
{
	struct ds3231_data *data = dev->driver_data;
	const struct register_map *rp = &data->registers;
...
static void alarm_worker(struct k_work *work)
{
	...
	LOG_DBG("ALARM %02x at %u latency %u", af, data->isw_syncclock,
		rtc_ds3231_read_syncclock(ds3231) - data->isw_syncclock);
}
			...
			int rc)
{
	...
	if (notify != NULL) {
		rtc_ds3231_notify_callback cb =
			(rtc_ds3231_notify_callback)sys_notify_finalize(notify, rc);
...
{
	struct ds3231_data *data = dev->driver_data;
	int rc = sc_ctrl(dev, 0U, RTC_DS3231_REG_CTRL_INTCN
			 | RTC_DS3231_REG_CTRL_RS_Msk);
...
int z_impl_rtc_ds3231_get_syncpoint(struct device *dev,
				    struct rtc_ds3231_syncpoint *syncpoint)
{
	struct ds3231_data *data = dev->driver_data;
	int rv = 0;
...
int rtc_ds3231_synchronize(struct device *dev,
			   struct sys_notify *notify)
{
	struct ds3231_data *data = dev->driver_data;
	int rv = 0;
...
int z_impl_rtc_ds3231_req_syncpoint(struct device *dev,
...
int rtc_ds3231_set(struct device *dev,
...
static int ds3231_init(struct device *dev)
{
	...
	rc = sc_ctrl(dev,
		     RTC_DS3231_REG_CTRL_INTCN,
		     RTC_DS3231_REG_CTRL_RS_Msk
		     | RTC_DS3231_ALARM1 | RTC_DS3231_ALARM2);
	if (rc < 0) {
...
int z_vrfy_rtc_ds3231_get_syncpoint(struct device *dev,
				    struct rtc_ds3231_syncpoint *syncpoint)
{
	struct rtc_ds3231_syncpoint value;
	int rv;
	...
	rv = z_impl_rtc_ds3231_get_syncpoint(dev, &value);
...
#include <syscalls/rtc_ds3231_get_syncpoint_mrsh.c>

int z_vrfy_rtc_ds3231_req_syncpoint(struct device *dev,
				    struct k_poll_signal *sig)
{
	Z_OOPS(Z_SYSCALL_SPECIFIC_DRIVER(dev, K_OBJ_DRIVER_COUNTER, ds3231_init));
	if (sig != NULL) {
	...
	return z_impl_rtc_ds3231_req_syncpoint(dev, sig);
}

#include <syscalls/rtc_ds3231_req_syncpoint_mrsh.c>
