...
static void finalize_cancel_locked(struct k_work *work)
{
	 ...
	 * what's waiting for the completion.
	 */
...
static int cancel_async_locked(struct k_work *work)
{
	 ...
	 * as canceling.
	 */
	int ret = k_work_busy_get(work);

	if (ret != 0) {
		atomic_set_bit(&work->flags, K_WORK_CANCELING_BIT);
		ret = k_work_busy_get(work);
	}
 ...
 *
 * @retval true iff the work was still active on entry.  The caller
...
{
	bool ret = atomic_test_bit(&work->flags, K_WORK_CANCELING_BIT);
...
static void work_queue_main(void *workq_ptr, void *p2, void *p3)
{
	...
	while (true) {
		...
		if (work == NULL) {
			...
			(void)z_sched_wait(&queue->lock, key, &queue->notifyq,
					   K_FOREVER, NULL);
			continue;
		}
		       ...
		       bool plug)
{
	...
	if (((atomic_get(&queue->flags)
		...
		notify_queue_locked(queue);
		ret = z_sched_wait(&queue->lock, key, &queue->drainq,
				   K_FOREVER, NULL);
	} else {
		k_spin_unlock(&queue->lock, key);
	}
 ...
 */
int schedule_to_queue_locked(struct k_work_q **queuep,
			       ...
			       k_timeout_t delay)
{
	...
	if (k_work_busy_get(work) == 0U) {
		ret = schedule_to_queue_locked(&queue, dwork, delay);
	} else {
		queue = NULL;
	}
