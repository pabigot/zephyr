...
static inline void z_vrfy_k_timer_start(struct k_timer *timer,
					...
					k_timeout_t period)
{
	Z_OOPS(Z_SYSCALL_VERIFY(K_TIMEOUT_GET(duration) >= 0
				&& K_TIMEOUT_GET(period) >= 0 &&
...
static inline k_ticks_t z_vrfy_k_timer_end_ticks(struct k_timer *timer)
{
	Z_OOPS(Z_SYSCALL_OBJ(timer, K_OBJ_TIMER));
	return z_impl_k_timer_end_ticks(timer);
}
#include <syscalls/k_timer_end_ticks_mrsh.c>
