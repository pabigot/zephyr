 ...
 * K_WORK_RUNNING, and K_WORK_CANCELLING.
 */
static inline int k_work_busy(const struct k_work *work);
 ...
 * @param work pointer to the work item.
 *
 * @return true if and only if k_work_busy() returns a non-zero value.
 */
 ...
 * @param work pointer to the work item.
 *
 * @param sync pointer to an opaque item containing state related to
 * the pending cancellation.  The object must persist until the call
 * returns, and be accessible from both the caller thread and the work
 * queue thread.
 *
 ...
 * @param work pointer to the work item.
 *
 * @return the k_work_busy() status indicating the state of the item after all
 * cancellation steps completed.
 */
 ...
 */
static inline k_tid_t k_work_queue_thread(struct k_work_q *queue);
 ...
 * K_WORK_CANCELLING.  A zero return value indicates the work item appears to
 * be idle.
 */
static inline int k_work_delayable_busy(const struct k_work_delayable *dwork);
 ...
 * @param dwork pointer to the delayable work item.
 *
 * @return true if and only if k_work_delayable_busy() returns a non-zero
 * value.
 */
static inline bool k_work_delayable_pending(
 ...
 * @param dwork pointer to the delayable work item.
 *
 * @return the k_busy_wait() state after cancellation initiated.
 */
 ...
 */
	 ...
	 * queue thread.
	 *
	 * Accessed via k_work_busy().  May co-occur with other flags.
	 */
...
struct k_work {
	 ...
	 *
	 * All changes are made under the work lock.
	 */
	atomic_t flags;
};
 ...
 *
 * Instances of this type are allocated on the stack during invocation
 * of  and sibling API.
 *
...
/** @brief A structure holding internal state for a pending
 * synchronous operation on a work item or queue.
 *
 * Instances of this type are provided by the caller for invocation of
 * k_work_flush(), k_work_cancel_sync() and sibling flush and cancel
 * APIs.  A referenced object must persist until the call returns, and
 * be accessible from both the caller thread and the work queue
 * thread.
 *
 * @note If CONFIG_KERNEL_COHERENCE is enabled the object should not
 * be stack-allocated.
 */
struct k_work_sync {
	union {
...
struct k_work_q {
	...
	/* Lock protecting pending list, flags, and drainq.
	 *
	 * This may be taken when the work lock is head.  No other locks may
...
static inline int k_work_busy(const struct k_work *work)
{
	return atomic_get(&work->flags) & K_WORK_MASK;
}

static inline bool k_work_pending(const struct k_work *work)
{
	return k_work_busy(work) != 0;
}
...
static inline bool k_work_delayable_pending(
	const struct k_work_delayable *dwork)
{
	return k_work_delayable_busy(dwork) != 0;
}

static inline k_ticks_t k_work_delayable_expires(
	const struct k_work_delayable *dwork)
{
	return z_timeout_expires(&dwork->timeout);
}

static inline k_ticks_t k_work_delayable_remaining(
	const struct k_work_delayable *dwork)
{
...
{
	bool pending = k_work_delayable_pending(&work->work);
...
{
	return k_work_delayable_pending(&work->work);
}

/* to be deprecated */
static inline int32_t k_delayed_work_remaining_get(struct k_delayed_work *work)
{
	k_ticks_t rem = k_work_delayable_remaining(&work->work);
...
static inline k_ticks_t k_delayed_work_expires_ticks(
	struct k_delayed_work *work)
{
	return k_work_delayable_expires(&work->work);
}
...
static inline k_ticks_t k_delayed_work_remaining_ticks(
	struct k_delayed_work *work)
{
	return k_work_delayable_remaining(&work->work);
}
