 ...
 * queue thread.  The object must not be used for any other flush or cancel
 * operation until this one completes.
 *
 ...
 * be idle.
 */
static inline int k_work_delayable_busy_get(const struct k_work_delayable *dwork);
...
struct k_work {
	 ...
	 *
	 * All changes are made under the work lock.  The state is
	 * atomic to ensure reads without lock are not optimized away.
	 */
	atomic_t flags;
};
