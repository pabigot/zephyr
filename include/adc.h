 ...
 *
 * @param dev Pinter to the device structure for the driver instance.
 *
 * @return The internal reference, in millivolts.
 * @retval -ENOTSUP if there is no internal reference
 */
static inline int adc_ref_internal(struct device *dev)
{
	int rv = -ENOTSUP;
	const struct adc_config_info *cip
		= (struct adc_config_info*)dev->config->config_info;
	if (cip) {
