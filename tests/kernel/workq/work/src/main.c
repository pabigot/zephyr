...
BUILD_ASSERT(COOP_PRIORITY < CONFIG_SYSTEM_WORKQUEUE_PRIORITY,
	     "COOP not higher than system workqueue");
BUILD_ASSERT(CONFIG_SYSTEM_WORKQUEUE_PRIORITY < CONFIG_ZTEST_THREAD_PRIORITY,
	     "System workqueue not higher than ZTEST");
...
/* We have two work queues: a cooperative one that's higher priority
 * than the test thread, and a preemptible one.  The test
 * infrastructure records the number of times each one executes in a
 * counter.
 *
...
static K_THREAD_STACK_DEFINE(coop_stack, STACK_SIZE);
static struct k_work_q coop_queue;
static atomic_t coop_ctr;
static inline int coop_counter(void)
{
	return atomic_get(&coop_ctr);
}
...
static void counter_handler(struct k_work *work)
{
	...
	if (k_current_get() == &coop_queue.thread) {
		atomic_inc(&coop_ctr);
...
static void test_unstarted(void)
{
	...
	zassert_equal(k_work_busy(&work), 0, NULL);

	rc = k_work_submit_to_queue(&coop_queue, &work);
	zassert_equal(rc, -ENODEV, NULL);
}

static void test_queue_start(void)
{
	static const char *const preempt_name = "wq.preempt";

	zassert_equal(preempt_queue.flags, 0, NULL);
	k_work_queue_start(&preempt_queue, preempt_stack, STACK_SIZE,
			    PREEMPT_PRIORITY, preempt_name);
	zassert_equal(preempt_queue.flags, K_WORK_QUEUE_STARTED, NULL);

	if (IS_ENABLED(CONFIG_THREAD_NAME)) {
		const char *tn = k_thread_name_get(&preempt_queue.thread);

		zassert_true(tn != preempt_name, NULL);
		zassert_true(tn != NULL, NULL);
		zassert_equal(strcmp(tn, preempt_name), 0, NULL);
	}

	k_work_queue_start(&coop_queue, coop_stack, STACK_SIZE,
			    COOP_PRIORITY, "wq.coop");
	zassert_equal(coop_queue.flags, K_WORK_QUEUE_STARTED, NULL);
}
...
static void test_1cpu_simple_queue(void)
{
	...
	k_work_init(&work, counter_handler);
	zassert_equal(k_work_busy(&work), 0, NULL);
	zassert_equal(k_work_pending(&work), false, NULL);

	/* Submit to the cooperative queue */
	rc = k_work_submit_to_queue(&coop_queue, &work);
	zassert_equal(rc, 1, NULL);
	zassert_equal(k_work_busy(&work), K_WORK_QUEUED, NULL);
	zassert_equal(k_work_pending(&work), true, NULL);

	/* Shouldn't have been started since test thread is
	 * cooperative.
	 */
	zassert_equal(coop_counter(), 0, NULL);

	/* Let it run, then check it finished. */
	k_sleep(K_TICKS(1));
	zassert_equal(coop_counter(), 1, NULL);
	zassert_equal(k_work_busy(&work), 0, NULL);
...
static void test_smp_simple_queue(void)
{
	...
	} while (k_work_pending(&work) && (delay < DELAY_MS));
...
static void test_1cpu_sync_queue(void)
{
	...
	k_sleep(K_TICKS(1));
	zassert_equal(coop_counter(), 0, NULL);
	zassert_equal(k_work_busy(&work), K_WORK_RUNNING, NULL);
...
static void test_1cpu_reentrant_queue(void)
{
	...
	handler_release();
	k_sem_take(&sync_sem, K_FOREVER);
	zassert_equal(coop_counter(), 2, NULL);
}
...
static void test_1cpu_queued_flush(void)
{
	 ...
	 */
	zassert_equal(k_work_busy(&work), K_WORK_QUEUED, NULL);
	zassert_true(k_work_flush(&work, &sync), NULL);

	/* Verify completion. */
	zassert_equal(coop_counter(), 1, NULL);
	zassert_true(!k_work_pending(&work), NULL);
	rc = k_sem_take(&sync_sem, K_NO_WAIT);
	zassert_equal(rc, 0, NULL);

	/* After completion flush should be a no-op */
	zassert_false(k_work_flush(&work, &sync), NULL);
}
...
static void test_1cpu_delayed_flush(void)
{
	...
	zassert_true(k_work_flush_delayable(&dwork, &sync), NULL);
...
static void test_1cpu_queued_cancel_sync(void)
{
	...
	/* Cancellation should complete immediately. */
	zassert_false(k_work_cancel_sync(&work, &sync), NULL);
...
static void test_1cpu_delayed_cancel_sync(void)
{
	...
	/* Cancellation should complete immediately. */
	zassert_false(k_work_cancel_delayable_sync(&dwork, &sync), NULL);
...
static void test_running_cancel_cb(struct k_timer *timer)
{
	...
	ctx->busy_rc = k_work_busy(&ctx->work);
	ctx->submit_rc = k_work_submit_to_queue(&coop_queue, &ctx->work);
	handler_release();
}
...
static void test_1cpu_running_cancel(void)
{
	...
	/* Submit to the cooperative queue. */
	rc = k_work_submit_to_queue(&coop_queue, wp);
	zassert_equal(rc, 1, NULL);
	zassert_equal(coop_counter(), 0, NULL);
	...
	zassert_true(k_work_cancel_sync(wp, &sync), NULL);
...
static void test_smp_running_cancel(void)
{
	...
	do {
		delay = k_ticks_to_ms_floor32(k_uptime_ticks() - ts0);
	} while ((k_work_busy(&work) != K_WORK_RUNNING)
		 && (delay < DELAY_MS));
	...
	/* Sync should wait. */
	zassert_equal(k_work_cancel_sync(&work, &sync), true, NULL);
...
static void test_drain_empty(void)
{
	...
	rc = k_work_queue_drain(&coop_queue, false);
	zassert_equal(rc, 0, NULL);
}
...
static void test_1cpu_plugged_drain(void)
{
	...
	rc = k_work_queue_drain(&coop_queue, true);
	zassert_equal(rc, 1, NULL);
	...
	zassert_equal(coop_queue.flags,
		      K_WORK_QUEUE_STARTED | K_WORK_QUEUE_PLUGGED,
		      NULL);
	...
	/* Unplug the queue */
	rc = k_work_queue_unplug(&coop_queue);
	zassert_equal(rc, 0, "rc %d", rc);
	zassert_equal(coop_queue.flags, K_WORK_QUEUE_STARTED,
		      NULL);
...
static void test_1cpu_basic_schedule(void)
{
	...
	zassert_equal(k_work_delayable_busy(&dwork), rc, NULL);
	zassert_equal(k_work_delayable_pending(&dwork), true, NULL);

	/* Scheduling again does nothing. */
	rc = k_work_schedule_for_queue(&coop_queue, &dwork, K_NO_WAIT);
	zassert_equal(rc, 0, NULL);
...
static void test_1cpu_basic_reschedule(void)
{
	...
	rc = k_work_reschedule_for_queue(&preempt_queue, &dwork,
					  K_MSEC(2U * DELAY_MS));
	zassert_equal(rc, 1, NULL);
	zassert_equal(k_work_busy(wp), K_WORK_DELAYED, NULL);
	...
	k_sleep(K_TICKS(1));
	sched_ms = k_uptime_get_32();
	rc = k_work_reschedule_for_queue(&coop_queue, &dwork,
					  K_MSEC(DELAY_MS));
	zassert_equal(rc, 1, NULL);
	zassert_equal(k_work_busy(wp), K_WORK_DELAYED, NULL);
...
static void test_1cpu_immed_reschedule(void)
{
	...
	rc = k_work_reschedule_for_queue(&coop_queue, &dwork, K_NO_WAIT);
	zassert_equal(rc, 1, NULL);
	zassert_equal(k_work_busy(wp), K_WORK_QUEUED, NULL);
	...
	k_sleep(K_TICKS(1));
	zassert_equal(coop_counter(), 0, NULL);
	zassert_equal(k_work_busy(wp), K_WORK_RUNNING, NULL);
	...
	zassert_equal(rc, 2, NULL);
	zassert_equal(k_work_busy(wp), K_WORK_QUEUED | K_WORK_RUNNING,
		      NULL);
	 ...
	 */
	zassert_equal(coop_counter(), 1, NULL);
	zassert_equal(k_work_busy(wp), K_WORK_DELAYED | K_WORK_RUNNING,
		      NULL);
	...
	zassert_equal(k_work_busy(wp), K_WORK_DELAYED,
		      NULL);
	...
	zassert_equal(k_work_busy(wp), K_WORK_RUNNING,
		      NULL);
...
static void test_1cpu_legacy_delayed_submit(void)
{
	...
	k_sleep(K_TICKS(1));
	sched_ms = k_uptime_get_32();
	rc = k_delayed_work_submit_to_queue(&coop_queue, &lwork,
					    K_MSEC(DELAY_MS));
	zassert_equal(rc, 0, NULL);
	...
	/* Make sure it ran and is now idle */
	zassert_equal(coop_counter(), 1, NULL);
	rc = k_work_delayable_busy(&lwork.work);
