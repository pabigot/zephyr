...
void test_all_channels_instance(const char *dev_name)
{
	...
	for (int i = 0; i < n; i++) {
		...
		if ((err == 0) && !limit_reached) {
			...
			zassert_equal(0, 1,
				      "%s: Unexpected error on setting alarm: %d", dev_name, err);
		}
	}
...
static void test_ds3231_synchronize(void)
{
	...
	rc = rtc_ds3231_synchronize(dev, &notify);
	zassert_true(rc >= 0,
...
static void test_ds3231_get_syncpoint(void)
{
	...
	rc = rtc_ds3231_get_syncpoint(dev, &syncpoint);
	zassert_true(rc >= 0,
...
static void test_ds3231_req_syncpoint(void)
{
	...
	k_poll_signal_reset(&sync_sig);
	rc = rtc_ds3231_req_syncpoint(dev, &sync_sig);
	zassert_true(rc >= 0,
