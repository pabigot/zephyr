...
void test_eeprom_slave(void)
{
	...
	LOG_INF("Found EP0 %s on I2C Master device %s at addr %02x",
		label_eeprom0, DT_BUS_LABEL(NODE_EP0), reg_addr0);
	...
	LOG_INF("Found EP1 %s on I2C Master device %s at addr %02x",
		label_eeprom1, DT_BUS_LABEL(NODE_EP1), reg_addr1);
	...
	zassert_equal(0, run_full_read(i2c_0, reg_addr0, eeprom_0_data),
		     "Full I2C read from EP0 failed");
	zassert_equal(0, run_full_read(i2c_1, reg_addr1, eeprom_1_data),
		     "Full I2C read from EP1 failed");

	for (offset = 0 ; offset < TEST_DATA_SIZE-1 ; ++offset) {
		zassert_equal(0, run_partial_read(i2c_0, reg_addr0,
			      eeprom_0_data, offset),
			      "Partial I2C read EP0 failed");
	}

	for (offset = 0 ; offset < TEST_DATA_SIZE-1 ; ++offset) {
		zassert_equal(0, run_partial_read(i2c_1, reg_addr1,
			      eeprom_1_data, offset),
			      "Partial I2C read EP1 failed");
	}

	for (offset = 0 ; offset < TEST_DATA_SIZE-1 ; ++offset) {
		zassert_equal(0, run_program_read(i2c_0, reg_addr0, offset),
			      "Program I2C read EP0 failed");
	}

	for (offset = 0 ; offset < TEST_DATA_SIZE-1 ; ++offset) {
		zassert_equal(0, run_program_read(i2c_1, reg_addr1, offset),
