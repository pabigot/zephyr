...
void test_power_states(void)
{
	enum pm_state dts_states[] =
		DT_PM_STATE_ITEMS_LIST(DT_NODELABEL(power_states));
	struct pm_state_info dts_infos[] =
		DT_PM_STATE_INFO_ITEMS_LIST(DT_NODELABEL(power_states));
	uint32_t dts_states_len =
		DT_PM_STATE_ITEMS_LEN(DT_NODELABEL(power_states));
